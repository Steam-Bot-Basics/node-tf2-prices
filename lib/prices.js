...
Prices.prototype._findUpdates = function(prices) {
    ...
    this.prices.forEach(function(price) {
        ...
        } else if (JSON.stringify(price.price.buy) != JSON.stringify(match.price.buy) || JSON.stringify(price.price.sell) != JSON.stringify(match.price.sell)) {
            // Found a match, and the prices have changed.
            self.emit('price', 2, price.item, match.price);
            found.push(match);
        } else {
