...
Prices.prototype._findUpdates = function(newPrices, oldPrices) {
    var self = this;
    ...
    newPrices.forEach(function(price) {
        var match = findItem(price.item.name, oldPrices);
        if (match == null) {
    ...
    if (newPrices.length - found.length > 0) {
        var count = newPrices.length - found.length;
        // Loop the current pricelist
        for (var i = 0; i < prices.length; i++) {
            if (count == 0) {
                break;
            }
            var item = newPrices[i].item;
            ...
            if (match == null) {
                self.emit('price', 1, newPrices[i].item, newPrices[i].price);
                count--;
            }
        }
    }
